...
class TotalLoss(nn.modules.loss._Loss):
    ...
    def forward(self, model_id, outputs, label_id, log=None, **kwargs):
        ...
        for source_id, (source_output, loss_func) in enumerate(
            zip(outputs, self.loss_funcs)
        ):
            kwargs["_source_id"] = source_id
            kwargs["_target_id"] = model_id
            losses += [loss_func(target_output, source_output, label_id, log, **kwargs)]
        total_loss = torch.stack(losses).sum()

        if log is not None:
            for source_id, (func, loss) in enumerate(zip(self.loss_funcs, losses)):
                log["ite_log"][self.ite][
                    f"{model_id:02}_{source_id:02}_" + func.__class__.__name__
                ] = float(loss)
...
class KLLoss(_LossBase):
    ...
    def forward(self, target_output, source_output, label_id, log=None, **kwargs):
        ...
        soft_loss_per_sample = self.kl_divergence(
            student_softmax, teacher_softmax, log=log
        )
...
class LinearGate(_BaseGate):
    def f(self, loss_per_sample, log, **kwargs):
        ...
        if log is not None:
            ...
            log["ite_log"][self.parent.ite][
                f"{target_id:02}_{source_id:02}_linear_weight"
            ] = float(loss_weight)
...
class CorrectGate(_BaseGate):
    def f(self, loss_per_sample, log, **kwargs):
        ...
        if log is not None:
            ...
            target_id = kwargs["_target_id"]
            log["ite_log"][self.parent.ite][
                f"{target_id:02}_{source_id:02}_TT"
            ] = float(TT.sum())
            log["ite_log"][self.parent.ite][
                f"{target_id:02}_{source_id:02}_TF"
            ] = float(TF.sum())
            log["ite_log"][self.parent.ite][
                f"{target_id:02}_{source_id:02}_FT"
            ] = float(FT.sum())
            log["ite_log"][self.parent.ite][
                f"{target_id:02}_{source_id:02}_FF"
            ] = float(FF.sum())
